...
flags.DEFINE_integer("max_eval_steps", 100000, "Maximum number of eval steps.")
...
flags.DEFINE_bool(
    "LM", False, "Whether to operate unidirectional LM pretraining.")
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        lm_weights = get_lm_weights(input_target_ids, word2id, [
        ...
        masked_lm_weights = get_lm_weights(
            masked_lm_ids, word2id, ['[PAD]', '[UNK]'])
        ...
        (lm_loss, lm_example_loss, lm_log_probs) = get_lm_output(
            bert_config, model.get_origin_sequence_output(
            ), model.get_embedding_table(), input_target_ids,
            lm_weights)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(masked_lm_example_loss, masked_lm_log_probs, masked_lm_ids,
                          masked_lm_weights, lm_example_loss, lm_log_probs,
            ...
            wrong_mask_lm_label = tf.constant(
                value=-1, dtype=tf.int32, shape=masked_lm_ids.shape)
            unk_id = word2id['[UNK]']
            unk_tf = tf.constant(
                value=unk_id, dtype=tf.int32, shape=masked_lm_ids.shape)
            condition_mask_lm_tf = tf.equal(masked_lm_ids, unk_tf)
            new_mask_lm_labels = tf.where(
                condition_mask_lm_tf, wrong_mask_lm_label, masked_lm_ids)

            wrong_lm_label = tf.constant(
                value=-1, dtype=tf.int32, shape=input_target_ids.shape)
            ...
            new_lm_labels = tf.where(
                condition_lm_tf, wrong_lm_label, input_target_ids)

            eval_metrics = metric_fn(
                masked_lm_example_loss, masked_lm_log_probs, new_mask_lm_labels,
                masked_lm_weights, lm_example_loss, lm_log_probs, new_lm_labels, lm_weights,
            )
...
def get_masked_lm_output(bert_config, input_tensor, output_weights, output_type_weights, positions,
    ...
    with tf.variable_scope("cls/predictions"):
        ...
        logits_type = tf.matmul(
            input_tensor, output_type_weights, transpose_b=True)
        ...
        one_hot_type_labels = tf.one_hot(
            type_label_ids, depth=bert_config.vocab_type_size, dtype=tf.float32)
        ...
        type_per_example_loss = - \
            tf.reduce_sum(log_probs_type * one_hot_type_labels, axis=[-1])
        type_numerator = tf.reduce_sum(
            type_label_weights * type_per_example_loss)
    ...
    with tf.variable_scope("cls/predictions/addtype"):
        ...
        concat_input_tensor = tf.layers.dense(tf.concat([input_tensor, tf.squeeze(type_pre_embedding_output)], -1),
                                              units=bert_config.hidden_size,
...
def input_fn_builder(input_files,
    ...
    def input_fn(params):
        ...
        name_to_features = {
            ...
            "lm_input_ids": tf.FixedLenFeature([max_seq_length], tf.int64),
            "lm_target_ids": tf.FixedLenFeature([max_seq_length], tf.int64),
            "input_mask":
                tf.FixedLenFeature([max_seq_length], tf.int64),
            "segment_ids":
                tf.FixedLenFeature([max_seq_length], tf.int64),
            ...
            "masked_lm_ids":
                tf.FixedLenFeature([max_predictions_per_seq], tf.int64),
            "input_type_ids": tf.FixedLenFeature([max_predictions_per_seq], tf.int64),
            "masked_lm_weights":
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        output_eval_file = os.path.join(
            FLAGS.output_dir, "small_test_results.txt")
        with tf.gfile.GFile(output_eval_file, "w") as writer:
    ...
    if FLAGS.do_test:
        ...
        tf.logging.info("***** Test results *****")
        output_eval_file = os.path.join(
            FLAGS.output_dir, "small_id_eval_results.txt")
        with tf.gfile.GFile(output_eval_file, "w") as writer:
            for i, p in tqdm(enumerate(result)):
                writer.write(str(p['masked_pre']) + ' ' +
                             str(p['masked_tar']) + '\n')
                #writer.write(str(p['lm_pre']) + ' ' + str(p['lm_tar']) + '\n')
